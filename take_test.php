...
if(isset($_POST['submit_answer'])){
    // Fetch the quizGold and quizExp values from the quiz table
    $quiz_id = $_GET['quiz_id'];
    $quiz_info_query = mysqli_query($conn, "SELECT quizGold, quizExp FROM quiz WHERE quiz_id = '$quiz_id'");
    $quiz_info = mysqli_fetch_assoc($quiz_info_query);
    $quizGold = $quiz_info['quizGold'];
    $quizExp = $quiz_info['quizExp'];

    // Divide quizExp by 100 and remove the remainder
    $quizExpLevel = floor($quizExp / 100);

    // Fetch the student_id from the student_class_quiz table
    $student_id_query = mysqli_query($conn, "SELECT student_id FROM student_class_quiz WHERE class_quiz_id = '$class_quiz_id'");
    $student_id_data = mysqli_fetch_assoc($student_id_query);
    $student_id = $student_id_data['student_id'];

    // Update the gold and level columns in the student table
    $update_student_query = mysqli_query($conn, "UPDATE student SET gold = gold + '$quizGold', level = level + '$quizExpLevel' WHERE student_id = '$student_id'");
    if($update_student_query) {
        echo "Gold and level updated successfully!";
    } else {
        echo "Error updating gold and level!";
    }

    // Rest of your code for calculating score and displaying results goes here
}
?>
...
?>
	<input type="text" name="q-<?php echo $roww['quiz_question_id'];?>" placeholder="Type your answer here">
<?php
...
<script>
jQuery(document).ready(function(){
    var totalQuestions = <?php echo $x; ?>;
    var currentQuestion = 1;

    // Function to show the current question
    function showQuestion() {
        jQuery('.questions').hide();
        jQuery('#q_' + currentQuestion).show();
    }
...
<button onclick="return false;" qn="<?php echo $x;?>" class="prevq btn btn-warning" id="prev_<?php echo $x;?>"><i class="icon-arrow-left"></i> Previous Question</button>
