...
<?php include('session.php'); ?>
    ...
    jQuery("#change_password").submit(function(e){
        ...
        if (new_password != retype_password) {
            $.jGrowl("New passwords do not match", { header: 'Change Password Failed' });
        } else {
            ...
            $.ajax({
                ...
                success: function(response){
                    if(response === 'current_password_incorrect') {
                        $.jGrowl("Current password is incorrect", { header: 'Change Password Failed' });
                    } else if(response === 'password_change_success') {
                        $.jGrowl("Your password has been successfully changed", { header: 'Change Password Success' });
                        var delay = 2000;
                        setTimeout(function(){ window.location = 'dashboard_student.php' }, delay);
                    } else {
                        $.jGrowl("An error occurred. Please try again.", { header: 'Change Password Failed' });
                    }
                }
            });
        }
